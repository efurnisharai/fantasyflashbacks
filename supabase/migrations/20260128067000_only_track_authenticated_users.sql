-- Only track stats for authenticated (non-anonymous) users
-- Delete existing anonymous profiles and prevent new ones from being tracked

-- 1) Delete existing anonymous user profiles
DELETE FROM public.user_profiles 
WHERE provider = 'anonymous' OR provider IS NULL;

-- 2) Update the trigger to NOT create profiles for anonymous users
...
BEGIN
  ...
  -- Only create profiles for non-anonymous users (Google, Apple, etc.)
  IF provider_name = 'anonymous' THEN
    RETURN new;
  END IF;
  ...
  -- Create profile only for authenticated users
...
-- 3) Update save function to only track authenticated users
...
BEGIN
  ...
  -- Check if this is a new high score (only from authenticated users)
  -- We'll set this after checking each user
  ...
  LOOP
    ...
    -- Always insert game result for history
    ...
    -- Only update profile stats for authenticated users
    IF v_is_authenticated AND FOUND THEN
      ...
      -- Update user profile stats (profile should already exist from sign-in trigger)
